	...
	public class GCloudVoiceEngine : IGCloudVoice
	{
		...
		public NoticeMessage NoticeMessage_ParseFrom(byte[] buf, int buflen)
		{
			...
			if (strlen == 0) {
				...
				pollMsg.strArg = System.Text.Encoding.UTF8.GetString( bstr );
			}
			guard += strlen;
		...
		{	
			...
			if (msg.what == (int)NoticeMessageType.MSG_ON_JOINROOM_COMPLETE) {
				...
				if (OnUploadReccordFileComplete != null) {
					OnUploadReccordFileComplete ((IGCloudVoice.GCloudVoiceCompleteCode)msg.intArg1, msg.strArg, msg.custom != null ? System.Text.Encoding.UTF8.GetString(msg.custom, 0, msg.datalen) : "");
				} 
			} else if (msg.what == (int)NoticeMessageType.MSG_ON_DOWNFILE_COMPLETE) {
				if (OnDownloadRecordFileComplete != null) {
					OnDownloadRecordFileComplete ((IGCloudVoice.GCloudVoiceCompleteCode)msg.intArg1, msg.strArg, msg.custom != null ? System.Text.Encoding.UTF8.GetString(msg.custom, 0, msg.datalen) : "");
				} 
